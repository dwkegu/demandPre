...
from demandPre.src.dataprocess.dataProvider import CNNDataProvider
...
if __name__ == '__main__':
    # [batch, T, d, h, w c]
    nyt_config_lstm = {'input_shape': [None, 4, 24*7, 64, 64, 1], 'output_shape': [None, 1, 64, 64, 1],
                  'model_name': "nyc-taxi-STC-LSTM-4w-168h-step", 't_length': 4, 'T': 24 * 7, 'splits':[14640, 1440, 1440], 'offset':False}
    nyb_config_lstm = {'input_shape': [None, 28, 24, 16, 8, 1], 'output_shape': [None, 1, 16, 8, 1],
    ...
    nyt_config_cnn = {"input_shape":[None, 24 * 7, 64, 64, 2], 'output_shape': [None, 1, 64, 64, 1],
                      'model_name':"ntc-taxi-cnn-7d", "splits":[14640, 1440, 1440], 'offset':False}
    nyb_config_cnn = {'input_shape': [None, 24 * 7 * 4, 16, 8, 1], 'output_shape': [None, 1, 16, 8, 1],
    ...
    if model_select == "CubeCNN":
        dataset = CNNDataProvider(config.dataset_path + "/NYC14_M16x8_T60_NewEnd.h5", 16, m_config["input_shape"][1],
