...
class BasicConvLSTMCell(BasicRNNCell):
  ...
  """
    ...
    """
        # super(BasicConvLSTMCell, self).__init__(num_features)
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope or type(self).__name__,):  # "BasicLSTMCell"
            ...
            self.h_w_f = tf.get_variable("h_w_f", shape=[self.shape[0], self.shape[1], self.num_features],
                                         dtype=tf.float32)
            self.h_w_i = tf.get_variable("h_w_i", shape=[self.shape[0], self.shape[1], self.num_features],
                                         dtype=tf.float32)
            self.h_w_j = tf.get_variable("h_w_j", shape=[self.shape[0], self.shape[1], self.num_features],
                                         dtype=tf.float32)
            self.h_w_o = tf.get_variable("h_w_o", shape=[self.shape[0], self.shape[1], self.num_features],
                                         dtype=tf.float32)
            ...
            concat = _conv_linear([inputs], self.filter_size, self.num_features * 4, bias=True, scope_name='conv')

            # i = input_gate, j = new_input, f = forget_gate, o = output_gate
            # i, j, f, o = tf.split(3, 4, concat)
            i, j, f, o = tf.split(concat, 4, 3)
            f = f + tf.multiply(h, self.h_w_f[tf.newaxis, :, :, :])
            i = i + tf.multiply(h, self.h_w_i[tf.newaxis, :, :, :])
            j = j + tf.multiply(h, self.h_w_j[tf.newaxis, :, :, :])
            o = o + tf.multiply(h, self.h_w_o[tf.newaxis, :, :, :])
...
def _conv_linear(args, filter_size, num_features, bias, bias_start=0.0, scope_name=None):
  ...
  """
    ...
    with tf.variable_scope(scope_name) as scope:
        ...
        matrix = tf.get_variable('Matrix', [filter_size[0], filter_size[1], total_arg_size_depth, num_features],
                                 dtype=dtype)
